...
contract MultiSigWallet {
    ...
    modifier onlyWallet() {
        require(msg.sender == address(this), "MultiSigWallet: caller is not the wallet");
        _;
    }
    ...
    modifier confirmed(uint transactionId, address owner) {
        require(confirmations[transactionId][owner], "MultiSigWallet: transaction is not confirmed by owner");
        _;
    }

    modifier notConfirmed(uint transactionId, address owner) {
        require(!confirmations[transactionId][owner], "MultiSigWallet: transaction is already confirmed by owner");
        _;
    }

    modifier notExecuted(uint transactionId) {
        require(!transactions[transactionId].executed, "MultiSigWallet: transaction is already executed");
        _;
    }
    ...
    constructor(
        ...
        for (uint i = 0; i < _owners.length; i++) {
            require(!isOwner[_owners[i]] && _owners[i] != address(0), "MultiSigWallet: invalid owner");
